...
class USBDevice_GD32F30x {
public:
	USBDevice_GD32F30x() : usb(USB->DEVICE) {
		// Empty
	}
...
private:
	...
	// TODO - this is a SAM platform device 
	// UsbDevice &usb;

	// Endpoints descriptors table
	// TODO - this is a sam platfrom thing:
	// 	__attribute__((__aligned__(4)))	UsbDeviceDescriptor EP[USB_EPT_NUM];
};

void USBDevice_GD32F30x::reset() {
#if 0
...
void USBDevice_GD32F30x::calibrate() {
#if 0
...
public:
	...
	DoubleBufferedEPOutHandler(USBDevice_GD32F30x &usbDev, uint32_t endPoint) :
		usbd(usbDev),
		ep(endPoint),
